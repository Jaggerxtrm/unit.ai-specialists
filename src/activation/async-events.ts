...
import { composeInteractionMessage, type InteractionMessage } from './interaction.js';
 ...
 */
export class ResultNotValidatedError extends Error {
  constructor(public readonly activationId: ActivationId) {
    super(
      `activation "${activationId}" has no validated result — a completion notification is a ` +
      'projection of ActivationResult and cannot be pushed before one exists',
    );
 ...
 */
export class RuntimeEventPusher {
  private readonly adapter: PeerAdapter;
  private readonly now: () => number;
  private readonly newMessageId: () => string;
  private readonly onPush?: (message: InteractionMessage, result: PushResult) => void;

  private readonly routes = new Map<ActivationId, EventRoute>();
  private readonly results = new Map<ActivationId, ActivationResult>();

  constructor(options: RuntimeEventPusherOptions) {
    this.adapter = options.adapter;
    this.now = options.now ?? (() => Date.now());
    this.newMessageId = options.newMessageId ?? (() => `msg:${randomUUID().slice(0, 12)}`);
   ...
   */
  async pushCompletion(activationId: ActivationId): Promise<PushResult> {
    const result = this.results.get(activationId);
    if (!result) throw new ResultNotValidatedError(activationId);
 ...
 */
export function completionBody(result: ActivationResult): string {
  return JSON.stringify(result);
}

/** Recover the projected result from a pushed body. Undefined if the body is not one. */
export function parseCompletionBody(body: string): ActivationResult | undefined {
  try {
    const parsed = JSON.parse(body) as Partial<ActivationResult>;
    if (typeof parsed?.activationId !== 'string' || typeof parsed?.status !== 'string') return undefined;
    return parsed as ActivationResult;
  } catch {
    return undefined;
  }
}
